...
class List extends React.Component {
  ...
  onDragEnter(event) {
    if (
      !event.dataTransfer.types.includes('application/tab') ||
      !this.props.customizable
    ) {
      return;
    }
  ...
  render() {
    const GroupComponent = this.props.customizable
      ? GroupWithDragAndDrop
      : Group;
    ...
    this.props.groups.forEach(group => {
      ...
      group.tabs.forEach(tab => {
        if (
          !this.state.tabInTransit ||
          tab.url !== this.state.tabInTransit.url
        ) {
          tabs.push(
            <Tab
              key={tab.url}
    ...
    return (
      <div
        ...
        onDrop={this.onDrop}
      >
        {this.state.dragOverCount > 0 ? (
          <NewGroup onDrop={this.createNewGroup} />
        ) : (
          ''
        )}
        {groups}

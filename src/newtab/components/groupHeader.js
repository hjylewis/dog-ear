...
class GroupHeader extends React.Component {
  ...
  render() {
    ...
    let groupName =
      this.props.data.group !== null ? (
        <span className="group-name__text">{this.props.data.group}</span>
      ) : (
        <div className="group-name__text group-name__text--no-category">
          <span className="group-name__text__maintext">Uncategorized</span>
          <span className="group-name__text__subtext">
            {' '}
            — Drag to organize into categories
          </span>
        </div>
      );
